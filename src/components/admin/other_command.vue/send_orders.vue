...
const fetch_product = async (status_orders) => {
    try {
        ...
        const { data } = await axios.get('/api/admin/all_order', { params: {status: status_orders},
            withCredentials: true
        })
        date.value = data
...
<template>
    <div class="flex flex-col gap-4 w-full">
        <div class="flex flex-col py-3 px-6 gap-4 shadow-md">
            <h1 v-if="date == 0" class="text-2xl font-bold text-center">Відправлених замовлень немає</h1>
            <div v-for="item in date" class="w-full">
                <div @click="item.show = !item.show"
                    ...
                    <div class="flex right-0 gap-2">
                        <button @click.stop="close_order(item.order_code)" class="bg-red-500 rounded p-2 text-white text-lg font-sans
                ...
                <div v-if="item.show"
                    ...
                    <cloud alt="Product Image" class="w-24" :fullurl="product.image_url" />
                    <div class="flex grid grid-cols-3 gap-4 justify-center content-center items-center">
                        ...
                        <div class="flex gap-2 text-center px-5">
                            <p class="text-lg">Дата відправки:</p>
                            <h1 class="text-lg font-bold">{{ product.date_dispatch }} </h1>
                        </div>
                    </div>
                </div>
            </div>
